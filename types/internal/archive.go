...
func init() {
	...
	types.RegisterSignature(types.Kind7ZipArchive, types.TypeNone, 0, []byte{
		'7', 'z', 0xbc, 0xaf, 0x27, 0x1c,
	})

	types.RegisterSignature(types.KindRARArchive, types.TypeRAR4, 0, []byte{
		'R', 'a', 'r', '!', 0x1a, 0x07, 0x00,
	})

	types.RegisterSignature(types.KindRARArchive, types.TypeRAR5, 0, []byte{
	...
	types.RegisterSignature(types.KindZstandardArchive, types.TypeNone, 0, []byte{
		0x28, 0xb5, 0x2f, 0xfd,
	})

	types.RegisterSignature(types.KindLZ4Frame, types.TypeNone, 0, []byte{
		0x04, 0x22, 0x4d, 0x18,
	})
	...
	types.RegisterSignature(types.KindSnappyFramedData, types.TypeNone, 0, []byte{
